...
export const metadata = generatePageMetadata({
  title: "Annual Return :: Ratnakar Securities Limited.",
  description:
...
export default async function AnnualReturnPage() {
  let groupedData = [];
  try {
    const response = await getInvestorDocuments("Annual Return");
    if (response.success && response.data && Array.isArray(response.data.grouped)) {
      groupedData = response.data.grouped;
    }
  } catch (error) {
    console.error("Error fetching annual return:", error);
  }
        ...
        <Container>
          <div className="mb-12 text-center">
            ...
            <p className="text-[16px] text-gray-700 max-w-3xl mx-auto font-medium">
              View and download our annual returns and related documents.
            </p>
          </div>
                      ...
                      group.files.map((file) => {
                        ...
                        return (
                          <a
                            ...
                            download
                            aria-label={`Download document: ${file.caption || "Annual Return Form"}`}
                            className="group p-6 border border-slate-200 rounded-2xl bg-white shadow-sm transition-all duration-300 hover:shadow-xl hover:shadow-[#a7181e]/10 hover:border-[#a7181e]/30 flex flex-col items-center text-center h-full focus-visible:ring-2 focus-visible:ring-[#a7181e] focus-visible:outline-none"
                          >
