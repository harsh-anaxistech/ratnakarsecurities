...
export default function AboutOverview() {
  ...
  // Dynamically calculate visible items based on real available screen real-estate
  useEffect(() => {
    const handleResize = () => {
      ...
      if (width < 768) {
        ...
        setVisibleCount(3); // Show fewer cards on medium/smaller laptops so nothing cuts off
      } else if (width < 1600) {
        setVisibleCount(4);
      } else {
        setVisibleCount(5); // Show full 5 cards only on huge desktop setups
      }
    };
  ...
  // Guard rails for resizing screens safely
  useEffect(() => {
    if (index > maxIndex) {
      setIndex(maxIndex);
    }
      ...
      <div className="py-10">
        ...
        <div className="relative h-64 md:h-80 rounded-sm overflow-hidden  flex items-center p-8 md:p-12">
          <div
            ...
            style={{
              backgroundImage: "url('/images/about/Milestones-Ratnakarsec.jpg')",
            }}
          />
