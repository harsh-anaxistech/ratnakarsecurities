        ...
        <div className="flex flex-col lg:flex-row gap-10">
          ...
          <aside className="w-full lg:w-[30%] space-y-8">
            <div className="rounded-2xl shadow-lg p-6 sticky top-[100px] bg-gradient-to-b from-[#2a689b] to-[#1e4b75] text-white">
              <h3 className="text-[18px] font-bold font-serif text-white mb-6 pb-4 border-b border-white/20 uppercase tracking-wide">Investment Options</h3>
              <ul className="space-y-3">
                {PRODUCTS_DATA.map((item) => (
                  <li key={item.id}>
                    <Link href={`/products/${item.slug}`} className={`flex items-center justify-between p-4 rounded-xl font-bold text-[16px] ${item.slug === slug ? "bg-white text-[#ea2830]" : "bg-white/10"}`}>
