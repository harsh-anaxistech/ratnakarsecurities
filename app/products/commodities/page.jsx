...
export default function ProductDetailsPage() {
  ...
  return (
    ...
    image="/images/about/23515 (1).jpg"
              ...
              <div className="space-y-4">
                {faqs.map((faq, idx) => (
                  <div key={idx} className="rounded-xl border border-black/5 bg-slate-50 overflow-hidden">
                    <button onClick={() => toggleFAQ(idx)} className="w-full p-5 text-left flex items-center justify-between font-bold text-slate-900">{faq.q} {openIndex === idx ? <Minus className="w-5 h-5" /> : <Plus className="w-5 h-5" />}</button>
            ...
            <div className="rounded-2xl shadow-lg p-6 sticky top-[100px] bg-gradient-to-b from-[#2a689b] to-[#1e4b75] text-white">
              ...
              <ul className="space-y-3">
                {PRODUCTS_DATA.map((item) => (
                  <li key={item.id}>
                    <Link href={`/products/${item.slug}`} className={`flex items-center justify-between p-4 rounded-xl font-bold ${item.slug === slug ? "bg-white text-[#ea2830]" : "bg-white/10 hover:bg-white hover:text-[#ea2830]"}`}>
                      {item.title} <ChevronRight className="w-5 h-5" />
                    </Link>
                  </li>
                ))}
              </ul>
