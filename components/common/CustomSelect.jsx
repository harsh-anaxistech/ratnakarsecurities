...
export default function CustomSelect({
  ...
  return (
    <div className="relative w-full" ref={containerRef}>
      ...
      >
        {Icon && (
          <div className="absolute left-4 text-slate-600 pointer-events-none z-10">
            <Icon className="w-5 h-5" aria-hidden="true" />
          </div>
        )}

        <span className={`block truncate ${selectedOption ? "text-slate-900 font-semibold" : "text-slate-600 font-medium"}`}>
          {selectedOption ? selectedOption.label : placeholder}
        </span>

        <div className="absolute right-4 pointer-events-none text-slate-600 z-10">
          <ChevronDown
            className={`w-4 h-4 transition-transform duration-200 ${isOpen ? "rotate-180 text-[#005a9c]" : ""}`}
            aria-hidden="true"
          />
        </div>
      </button>
              ...
              return (
                <li
                  ...
                  className={`px-4 py-2.5 text-sm sm:text-[15px] font-medium cursor-pointer transition-colors duration-150 flex items-center justify-between ${
                    isSelected
                      ? "bg-sky-50 text-[#005a9c] font-semibold"
                      : isFocused
                ...
                >
                  ...
                  {isSelected && (
                    <Check className="w-4 h-4 text-[#005a9c]" />
                  )}
                </li>
              );
            })}
